...
*select_texture(t_myray *ray, t_mygame *game)
{
	...
	/* helper: read a pixel from texture safely */
	static unsigned int
	tex_get_pixel(t_myimg *tex, int tx, int ty)
	{
		char            *ptr;
		unsigned int    pix;
	...
	*select_texture(t_myray *ray, t_mygame *game)
	...
	/* draw a flat colored vertical stripe (used as fallback) */
	static void
	draw_flat_column(t_mygame *game, int x, int y0, int y1, int side)
	{
		int color;
		int yy;
		...
		while (yy < y1)
		{
			draw_pixel_to_buffer(&game->img, x, yy, color);
			yy++;
		}
	}
	...
	draw_textured_column(t_myray *ray, t_mygame *game, int x)
	{
		...
		if (tex == NULL || tex->addr == NULL || tex->width == 0 || tex->height == 0)
		{
			draw_flat_column(game, x, draw_start, draw_end, ray->side);
			return;
		}
